...
namespace GameServer
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            SrxRevo.Systemcore.Definitions.Bootlogo._Load();
            ...
            if (File.Exists("./Settings/Settings.ini"))
            {
                ...
                SrxRevo.Systems.maxSlots = Convert.ToInt32(ini.GetValue("Server", "MaxSlots", 100));
            }
            else
            {
                Systems.Rate.Gold = 1;
            ...
            SrxRevo.Systems.CheckDirectories();

            SrxRevo.File.FileLoad.Load();
            //Update serverlist info
            SrxRevo.Systems.clients.update += new EventHandler(Users.updateServerList);
            //Load random unique monsters.
            Random rand = new Random();

            SrxRevo.GlobalUnique.StartTGUnique(rand.Next(10, 20) * 60000, 600);   //Random spawn tiger girl
            SrxRevo.GlobalUnique.StartUriUnique(rand.Next(10, 20) * 60000, 600);   //Random spawn urichi
            SrxRevo.GlobalUnique.StartIsyUnique(rand.Next(10, 20) * 60000, 600);   //Random spawn isy
            SrxRevo.GlobalUnique.StartLordUnique(rand.Next(10, 20) * 60000, 600);   //Random spawn lord yarkan
            SrxRevo.GlobalUnique.StartDemonUnique(rand.Next(10, 20) * 60000, 600);   //Random spawn demon shaitan
            SrxRevo.GlobalUnique.StartCerbUnique(rand.Next(10, 20) * 60000, 600);   //Random spawn cerberus
            SrxRevo.GlobalUnique.StartMedusa(rand.Next(10, 20) * 90000, 600);   //Random spawn medusa
            SrxRevo.GlobalUnique.StartNeith(rand.Next(10, 20) * 90000, 600);   //Random spawn neith
            //Game.GlobalUnique.StartSphinx       (rand.Next(10, 20) * 90000, 600);   //Random spawn medusa
            SrxRevo.GlobalUnique.StartIsis(rand.Next(10, 20) * 90000, 600);   //Random spawn isis
            //Game.GlobalUnique.StartRoc          (rand.Next(10, 20) * 90000, 600);   //Random spawn roc
            SrxRevo.GlobalUnique.StartIvyUnique(rand.Next(10, 20) * 60000, 600);   //Random spawn captain ivy
                    ...
                    if (remoteLoginServer != null)
                    {
                        ...
                        if (data.Length >= (dLen + 6))
                        {
                            if (pCmd == (byte)IPCCommand.IPC_REQUEST_SERVERINFO)
                            {
                                ...
                                byte[] rspBuf = Systems.IPC.PacketResponseServerInfo((UInt16)Servers.IPCPort, 1, 100, (UInt16)Systems.clients.Count, (UInt16)SrxRevo.Global.Versions.clientVersion);
            ...
            try
            {
                if (aCommand != null)
                {
                    ...
                    else if (command[0] == "//shutdown")
                    {
                        ...
                        {
                            Console.WriteLine("{0}stopping server and sending notice to clients ...", SrxRevo.Global.Product.Prefix);
                    ...
                    {
                        SrxRevo.GlobalUnique.StartTGUnique(6000 * 10, 6000 * 10);
                        SrxRevo.GlobalUnique.StartUriUnique(7000 * 10, 7000 * 10);
                        SrxRevo.GlobalUnique.StartIsyUnique(8000 * 10, 8000 * 10);
                        SrxRevo.GlobalUnique.StartLordUnique(9000 * 10, 9000 * 10);
                        SrxRevo.GlobalUnique.StartDemonUnique(10000 * 10, 10000 * 10);
                        SrxRevo.GlobalUnique.StartCerbUnique(11000 * 10, 11000 * 10);
                        SrxRevo.GlobalUnique.StartIvyUnique(11000 * 10, 11000 * 10);
                        //Game.GlobalUnique.StartRoc(11000 * 10, 11000 * 10);
                        SrxRevo.GlobalUnique.StartMedusa(11000 * 10, 11000 * 10);
                        SrxRevo.GlobalUnique.StartNeith(11000 * 10, 11000 * 10);
                        //Game.GlobalUnique.StartSphinx(11000 * 10, 11000 * 10);
                        SrxRevo.GlobalUnique.StartIsis(11000 * 10, 11000 * 10);
                    ...
                    else if (command[0] == "")
                    {
                        ...
                        SrxRevo.Systems c = new SrxRevo.Systems();
                        SrxRevo.Systems.SendAll(c.sendnoticecon(7, 0, information, ""));
