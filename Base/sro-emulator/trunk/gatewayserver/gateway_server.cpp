...
GatewayServer::GatewayServer(uint16_t port, std::size_t thread_count)
	:   m_acceptor     (m_service)
{
	...
	m_acceptor.async_accept(client->getSocket(), boost::bind(&GatewayServer::OnAccept, this, boost::asio::placeholders::error, id));
};
/////////////////////////////////////////////////////////////////////////////////////////////////////


ClientContext* GatewayServer::get_next_client(uint32_t *id)
{
	*id = free_client_ident();
	ClientContext *context = new ClientContext(m_service);
	context->setID(*id);
	context->setDisconnectHandler(boost::bind(&GatewayServer::manage_disconnect, this, _1)); // todo: could use singleton design too.

	m_client_container.insert(std::pair<uint32_t, ClientContext*>(*id, context));
	return context;
};

uint32_t GatewayServer::free_client_ident()
{
		...
		{
			if(current_position > GatewayConfig::MaximalConnections())
			{
...
void GatewayServer::OnAccept(const boost::system::error_code& e, uint32_t id)
{
	// finish & check connection
	printl("Client connected to the server on slot ");
	std::cout << id << ". (" 
	...
	m_acceptor.async_accept(client->getSocket(), boost::bind(&GatewayServer::OnAccept, this, boost::asio::placeholders::error, id_2));
...
void GatewayServer::manage_disconnect(uint32_t id)
{
	try
	{
