...
private:
	...
	void SpawnPlayers();
	void OnItem();
	void OnEmotion();// update carlosx
	void OnSit(); // update carlosx
	...
	void OnEstrella(); //update carlosx
	void OnUpdateSkill(); //update skill
	void OnMastertSkill(); //update CarlosX
	void OnATSkill(); //update carlosx
	void OnADDStr(); //update carlosx
	void OnADDInt(); //update carlosx
	void SpawnMnstll(unsigned int id, unsigned char posx, unsigned char posy, float fx, float fz, float fy);
    void SpawnAllM();
    int onlineMob;
	
	//Monster
    void MovementMonster();
	/*int[] Mobst = new int[200];
	int MobHP;
	int MobID;
	int MobObjectID;
	int MobType;
